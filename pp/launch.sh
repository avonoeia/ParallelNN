...
TRAIN_ROOT_PATH="experiments/pp"
TRAIN_SCRIPT_PATH="$TRAIN_ROOT_PATH/train_pp.py"

# Define the command template
COMMAND_TEMPLATE="source ~/miniconda3/etc/profile.d/conda.sh && conda activate t1 && export MASTER_ADDR=$MASTER_ADDR && export MASTER_PORT=$MASTER_PORT && torchrun --nproc_per_node=1 --nnodes=${#nodes[@]} --node_rank=%d --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT ~/$TRAIN_SCRIPT_PATH"
...
for node_name in "${!nodes[@]}"; do
    ...
    if [[ "$node_ip" != *"$MASTER_ADDR"* ]]; then
        ...
        scp "$HOME/$TRAIN_ROOT_PATH/model.py" "$node_name@$node_ip:~/$TRAIN_ROOT_PATH/"
    fi
done
